...
public class ReaderPreference implements ReadersWriters
{
  private int data = 0;
  private int noReaders = 0, noWriters = 0, noWaitingReaders = 0;
  ...
  {
    while (noReaders > 0 || noWriters > 0 || noWaitingReaders > 0)
    {
      try
      {
    ...
    System.out.println(Thread.currentThread().getName() + " acquired write lock | writers count: " + noWriters);
    noWriters++;
  }
  ...
  {
    noWriters--;
    System.out.println(Thread.currentThread().getName() + " release write lock | writers count: " + noWriters);
